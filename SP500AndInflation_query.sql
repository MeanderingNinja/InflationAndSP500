-- This query is to create a new table that contains both S&P 500 index and inflation data 05/25/2022

CREATE TABLE sp500_index."SP500AndInflation" AS
-- Temp table (calculate inflation data from the cleaned CPI-U dataset)
WITH 
InflationTable AS
(SELECT 
  "Year",
...
-- temp table (format inflation data to 2 columns) like this:
-------------------------------
...
-- temp table(format CPI data to 2 columns) like this:
-------------------------------
...
ORDER BY
 ...
 -- Temp table (Join the above two temp tables)
CPIAndInflation AS
(SELECT 
...
FROM 
  ...
  -- Select both S&P500 index and inflation data 
  SELECT 
    sp500."Id",
    sp500."Month_Year" AS month_year, 
    sp500."Price" AS sp500_index,
    -- Queries below will create columns of text data type as there are null values
    CPIAndInflation.cpi_index AS cpi_index,
    CPIAndInflation.inflation_rate AS inflation_rate 
  FROM 
    sp500_index."SP500Index" AS sp500 
    INNER JOIN CPIAndInflation
    ON sp500."Month_Year" = CPIAndInflation.month_year;
